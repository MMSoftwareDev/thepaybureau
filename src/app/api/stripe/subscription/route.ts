...
export async function GET() {
  try {
    ...
    if (userError) console.error('User lookup failed in stripe/subscription:', userError)
    if (!user) {
    ...
    if (tenantError) console.error('Tenant lookup failed in stripe/subscription:', tenantError)
    if (!tenant) {
