  ...
  try {
    ...
    if (template?.data) {
      ...
      if (!clientError && insertedClients) {
        ...
        // Create demo onboarding for onboarding clients
        const onboardingClient = insertedClients.find((c: any) => c.status === 'onboarding')
        ...
        if (onboardingClient) {
          ...
          if (onboardingTemplate) {
            // Set some tasks as completed for demo
            const demoTasks = onboardingTemplate.tasks.map((task: any, index: number) => ({
              ...task,
            ...
            console.log('✅ Demo onboarding created')
          }
        }
      }
    }
